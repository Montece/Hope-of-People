...
public class TOD_Time : MonoBehaviour
{
	...
	{
		bool bTimeProgress = this.gTime.bTimeProgress;
		if (this.nUsePersistentTime == 1)
		{
			this.DayLengthInMinutes = this.gTime.fDayLengthMinutes;
			this.sky.Cycle.Hour = this.gTime.fHourCurrent;
			this.sky.Cycle.Day = this.gTime.nDayCurrent;
			this.sky.Cycle.Month = this.gTime.nMonthCurrent;
			this.sky.Cycle.Year = this.gTime.nYearCurrent;
		}
		else if (bTimeProgress && this.DayLengthInMinutes > 0f)
		{
			float num = 1440f;
			float num2 = num / this.DayLengthInMinutes;
			this.AddSeconds(Time.deltaTime * num2, true);
		}
	}
