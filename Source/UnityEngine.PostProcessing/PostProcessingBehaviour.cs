...
{
	[AddComponentMenu("Effects/Post-Processing Behaviour", -1), DisallowMultipleComponent, ExecuteInEditMode, ImageEffectAllowedInSceneView, RequireComponent(typeof(Camera))]
	public class PostProcessingBehaviour : MonoBehaviour
	{
		...
		private void OnEnable()
		{
			...
			foreach (PostProcessingComponentBase current in this.m_Components)
			{
				this.m_ComponentStates.Add(current, false);
			}
		...
		private void OnPreCull()
		{
			...
			foreach (PostProcessingComponentBase current in this.m_Components)
			{
				if (current.active)
				{
					depthTextureMode |= current.GetCameraFlags();
				}
			}
		...
		{
			foreach (KeyValuePair<CameraEvent, CommandBuffer> current in this.m_CommandBuffers.Values)
			{
		...
		{
			foreach (KeyValuePair<PostProcessingComponentBase, bool> current in this.m_ComponentStates)
			{
				PostProcessingComponentBase key = current.Key;
				bool enabled = key.GetModel().enabled;
				if (enabled != current.Value)
				{
					if (enabled)
					{
		...
		private void DisableComponents()
		{
			foreach (PostProcessingComponentBase current in this.m_Components)
			{
				PostProcessingModel model = current.GetModel();
