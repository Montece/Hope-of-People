...
namespace UnityEngine.PostProcessing
{
	...
	public class ScreenSpaceReflectionModel : PostProcessingModel
	{
		...
		{
			[Range(0f, 2f), Tooltip("Nonphysical multiplier for the SSR reflections. 1.0 is physically based.")]
			public float reflectionMultiplier;

			[Range(0f, 1000f), Tooltip("How far away from the maxDistance to begin fading SSR.")]
			public float fadeDistance;

			[Range(0f, 1f), Tooltip("Amplify Fresnel fade out. Increase if floor reflections look good close to the surface and bad farther 'under' the floor.")]
			public float fresnelFade;

			[Range(0.1f, 10f), Tooltip("Higher values correspond to a faster Fresnel fade as the reflection changes from the grazing angle.")]
		...
		public struct ReflectionSettings
		{
			...
			[Range(0.1f, 300f), Tooltip("Maximum reflection distance in world units.")]
			public float maxDistance;

			[Range(16f, 1024f), Tooltip("Max raytracing length.")]
			public int iterationCount;

			[Range(1f, 16f), Tooltip("Log base 2 of ray tracing coarse step size. Higher traces farther, lower gives better quality silhouettes.")]
			public int stepSize;

			[Range(0.01f, 10f), Tooltip("Typical thickness of columns, walls, furniture, and other objects that reflection rays might pass behind.")]
			public float widthModifier;

			[Range(0.1f, 8f), Tooltip("Blurriness of reflections.")]
		...
		{
			[Range(0f, 1f), Tooltip("Higher = fade out SSRR near the edge of the screen so that reflections don't pop under camera motion.")]
